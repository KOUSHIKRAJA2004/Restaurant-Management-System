...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($results_capacity) {
        ...
        // Generate a simple 3-digit reservation ID based on the count of reservations for that date
        $date = $_POST["reservation_date"];
        $check_query = "SELECT COUNT(*) AS count FROM Reservations WHERE reservation_date = '$date'";
        $result = mysqli_query($link, $check_query);
        $row = mysqli_fetch_assoc($result);
        $count = $row['count'] + 1;
        $reservation_id = str_pad($count, 3, "0", STR_PAD_LEFT);
